...
Note: each fixture row's home/away teams are editable `<select>`s (not static text) — this is what lets admin "swap which teams are assigned" per the spec's Fixtures & schedule generation section, alongside the already-editable date. Both selects are scoped to `divisionTeams` (teams in the fixture's own division), consistent with the "Add Fixture Manually" form below them.
