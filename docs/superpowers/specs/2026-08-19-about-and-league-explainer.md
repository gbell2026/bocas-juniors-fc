...
- **Start**: `divisions[0].season_start_date` (the array is already fetched `order('season_start_date')`, so the first row is earliest — same assumption `getFixtureCalendar` already relies on elsewhere in this codebase)
...
Both are formatted for display via `Intl.DateTimeFormat` with `{ day: 'numeric', month: 'long', year: 'numeric' }`, in the current locale (`es-ES` or `en-GB`) — same formatting approach already used by `longDate` in `src/components/league/fixture-calendar.tsx`.
