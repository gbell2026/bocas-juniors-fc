...
`/admin`'s server-side data fetching (auth check + the `Promise.all` of every admin data source) stays exactly as-is — this is a presentation-layer change, not a data-layer change. The page becomes a thin server component that fetches data and passes it all to a new client component that owns the tab state and renders the right section. `user.id` (currently used inline for `<MediaUploader uploadedBy={user.id} />`) must be threaded through to the client wrapper alongside the fetched data.

### 3. Table-ify the two card-list sections

**Get Involved Submissions** (`src/components/admin/get-involved-submissions.tsx`): convert to a `<table>` with columns Name, Email, Organisation, Interests, Message, Date, Status/Action — matching the Players table's `<table>` styling (`bg-brand-creamAlt` header row, `border-t` row dividers, wrapped in `<div className="overflow-x-auto">` exactly as `players-table.tsx` does, since this table has 7 columns and needs the same horizontal-scroll safety net on narrow viewports). Long message text truncates with the full text available via a `title` attribute (no modal/expand — keep it simple, matching this codebase's existing preference for no unnecessary interaction layers). Null `organisation`/`message` render as an em dash, matching how other admin tables show missing optional data. "Mark as Handled" becomes a per-row action button; handled rows get the existing dimmed (`opacity-40`) treatment applied to the row instead of a card.

**Pending Submissions / Gallery** (`src/components/admin/pending-submissions.tsx`): convert to a `<table>` with columns Thumbnail, Submitter, Caption, Date, Actions (Approve/Reject side by side), also wrapped in `<div className="overflow-x-auto">`. Thumbnail stays the existing small Cloudinary-transformed image / video-icon placeholder, just inside a table cell instead of a card.
...
**Existing tests:** `src/__tests__/components/get-involved-submissions.test.tsx` and `pending-submissions.test.tsx` already cover both components and query by text/role rather than DOM structure — no card-specific selectors. They must keep passing unmodified through this card→table conversion; if a specific assertion turns out to depend on structure, adjust the test to match the new table markup rather than weakening the assertion.
