...
`payments.paid_at` is `timestamptz`; `finance_seasons.start_date`/`end_date` are `date`. The range check casts explicitly: `paid_at at time zone 'utc' >= start_date and paid_at at time zone 'utc' < end_date + 1` (half-open, so a payment on the end date itself is included). Pinning `at time zone 'utc'` explicitly, rather than relying on the query session's timezone setting, matches this codebase's established convention of never trusting ambient/session timezone for date comparisons (see the `timeZone: 'UTC'` requirement on every `Intl.DateTimeFormat` call, e.g. `src/components/upcoming-schedule.tsx`).
...
**Known limitation:** payment rows that existed when migration `005_registration_payment_plans.sql` ran were backfilled with `installment_label = 'full'` regardless of whether they represented a combined registration+season payment (see that migration's comments, lines 11-20). Rows inserted between migrations 005 and 010 default to `'full'` for the same reason — `'registration'` didn't exist as a valid label until 010 added it. Under this scheme, all such historical rows are counted entirely under Subscriptions, never Registration Fees — so a season's Registration Fees actual may be understated for any date range that includes pre-migration-010 payments. Acceptable for this build; not worth a data migration to fix retroactively.
